...
def build_model(dim, learn_rate, lmbda, drop, FL, init, n_filters):
    ...
    a1 = Conv2D(n_filters, FL, activation='relu', kernel_initializer=init,
                kernel_regularizer=l2(lmbda), padding='same')(img_input)
    a1 = Conv2D(n_filters, FL, activation='relu', kernel_initializer=init,
                kernel_regularizer=l2(lmbda), padding='same')(a1)
    a1P = MaxPooling2D((2, 2), strides=(2, 2))(a1)

    a2 = Conv2D(n_filters * 2, FL, activation='relu', kernel_initializer=init,
                kernel_regularizer=l2(lmbda), padding='same')(a1P)
    a2 = Conv2D(n_filters * 2, FL, activation='relu', kernel_initializer=init,
                kernel_regularizer=l2(lmbda), padding='same')(a2)
    a2P = MaxPooling2D((2, 2), strides=(2, 2))(a2)

    a3 = Conv2D(n_filters * 4, FL, activation='relu', kernel_initializer=init,
                kernel_regularizer=l2(lmbda), padding='same')(a2P)
    a3 = Conv2D(n_filters * 4, FL, activation='relu', kernel_initializer=init,
                kernel_regularizer=l2(lmbda), padding='same')(a3)
    a3P = MaxPooling2D((2, 2), strides=(2, 2),)(a3)

    u = Conv2D(n_filters * 4, FL, activation='relu', kernel_initializer=init,
               kernel_regularizer=l2(lmbda), padding='same')(a3P)
    u = Conv2D(n_filters * 4, FL, activation='relu', kernel_initializer=init,
               kernel_regularizer=l2(lmbda), padding='same')(u)

    u = UpSampling2D((2, 2))(u)
    u = Concatenate(axis=3)([a3, u])
    u = Dropout(drop)(u)
    u = Conv2D(n_filters * 2, FL, activation='relu', kernel_initializer=init,
               kernel_regularizer=l2(lmbda), padding='same')(u)
    u = Conv2D(n_filters * 2, FL, activation='relu', kernel_initializer=init,
               kernel_regularizer=l2(lmbda), padding='same')(u)

    u = UpSampling2D((2, 2))(u)
    u = Concatenate(axis=3)([a2, u])
    u = Dropout(drop)(u)
    u = Conv2D(n_filters, FL, activation='relu', kernel_initializer=init,
               kernel_regularizer=l2(lmbda), padding='same')(u)
    u = Conv2D(n_filters, FL, activation='relu', kernel_initializer=init,
               kernel_regularizer=l2(lmbda), padding='same')(u)

    u = UpSampling2D((2, 2))(u)
    u = Concatenate(axis=3)([a1, u])
    u = Dropout(drop)(u)
    u = Conv2D(n_filters, FL, activation='relu', kernel_initializer=init,
               kernel_regularizer=l2(lmbda), padding='same')(u)
    u = Conv2D(n_filters, FL, activation='relu', kernel_initializer=init,
    ...
    u = Conv2D(1, 1, activation=final_activation, kernel_initializer=init,
               kernel_regularizer=l2(lmbda), padding='same')(u)
    u = Reshape((dim, dim))(u)
