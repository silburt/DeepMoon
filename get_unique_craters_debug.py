...
#########################
def estimate_longlatdiamkm(dim, llbd, distcoeff, coords, i):
    ...
    if np.any(np.isnan(long_deg)):
        print("error in %d:"%i)
        print(list(zip(long_deg,long_pix,latdiff,lat_deg_firstest,np.sin(np.pi * latdiff / 180.))))
    ...
    for i in range(CP['n_imgs']):
        ...
        if len(coords) > 0:
            ...
            new_craters_unique = estimate_longlatdiamkm(
                dim, P[llbd][id], P[distcoeff][id][0], coords, i)
